...
CONFIG = QuestionConfig(
    question_id="btc-us-shutdown-ends",
    asset_id=1,
    convert_id=2781,
    min_decision_epoch=1762473600,  # 2025-11-07T00:00:00Z
    market_end_epoch=1763078400,  # 2025-11-14T00:00:00Z
    require_decision_input=True,
    default_decision_epoch=None,
)
...
def build_parser(config: QuestionConfig) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "prices 12–24 hours after the shutdown decision timestamp."
        )
    )
    parser.add_argument(
        "--decision-time",
        help=(
            "ISO8601 timestamp for when the shutdown actually ended (funding restored, "
            "OPM back to Open)."
        ),
    )
    parser.add_argument(
        "--decision-time-epoch",
        type=int,
        help="Unix timestamp seconds for the shutdown decision (alternative to --decision-time).",
    )
    parser.add_argument(
        "--artifacts",
        type=Path,
