...
def test_resolve_decision_epoch_requires_timestamp() -> None:
    with pytest.raises(MetricError):
        resolve_decision_epoch(make_args(), CONFIG)
...
def test_resolve_decision_epoch_rejects_future_times() -> None:
    args = make_args(decision_time_epoch=CONFIG.market_end_epoch + 1)
    with pytest.raises(MetricError):
        resolve_decision_epoch(args, CONFIG)
