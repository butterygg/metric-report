...
CONFIG = QuestionConfig(
    question_id="ftt-sbf-clemency-granted",
    asset_id=4195,
    convert_id=2781,
...
def build_parser(config: QuestionConfig) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compute the FTT/USD Reality metric by sampling CoinMarketCap detail/chart "
            "prices 12–24 hours after the clemency cutoff (or optional override)."
        )
    )
    parser.add_argument(
        "--decision-time",
        help=(
            "ISO8601 timestamp for when a White House clemency action is published. "
            "Defaults to the market cutoff when omitted."
        ),
    )
    parser.add_argument(
        "--decision-time-epoch",
        type=int,
        help="Unix timestamp seconds for the clemency decision (alternative to --decision-time).",
    )
    parser.add_argument(
        "--artifacts",
        type=Path,
