...
of USDC Aave V3 Ethereum supply APY excluding rewards (base APY),
...
Endpoint:
  https://yields.llama.fi/chart/aa70268e-4b52-42bf-a116-608b370f9501
"""
...
ENDPOINT = "https://yields.llama.fi/chart/aa70268e-4b52-42bf-a116-608b370f9501"
DATA_DIR = os.path.dirname(__file__)
RAW_PATH = os.path.join(DATA_DIR, "defillama_yields_usdc_aavev3_eth_raw.json")

# Trailing window definition
...
def parse_utc_date(ts_val) -> date:
    """Accepts integer seconds since epoch or ISO 8601 string and returns UTC date."""
    # Integer-like given as number or numeric string
    ...
    # ISO 8601 string, e.g. '2023-02-06T23:01:24.670Z' or with offset
...
def main() -> None:
    # Try to reuse existing raw data if present to avoid refetch; always prefer fresh fetch.
    try:
        payload = fetch_and_save_raw()
    except Exception as e:
        # Fallback to local data if network not available
    ...
    points = payload["data"]
    # Bucket APY base values by UTC calendar date within the window
    by_day: dict[date, list[float]] = defaultdict(list)

    for pt in points:
        # Expected keys: timestamp (sec), apyBase, apyReward, apy, etc.
        # Some datasets use 'timestamp', others use 'datetime' or similar; be flexible.
        ts = pt.get("timestamp")
        if ts is None:
            ts = pt.get("datetime") or pt.get("time")
        ...
        if START_DATE <= d <= END_DATE:
            # apyBase is expressed in percent units (e.g., 5.32 means 5.32%)
            try:
                val = float(apy_base)
            except Exception:
                continue
    ...
    # Construct per-day values for each day in the window, ensuring coverage
    ...
    if missing_days:
        # Report missing days; continue with available days but warn user
    ...
    # Compute average of daily values (each day equally weighted)
